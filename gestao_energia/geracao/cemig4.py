...
def extrairExpressaoRegular(expr: str, texto: str) -> []:
    ...
    print ('Ocorrências encontradas: '+str(len(ocorrencias)))
    #print (ocorrencias[0])
...
def extrairEnergiaInjetada(pag1: str) -> float:
    ...
    '''
    print ('Vou extrair a energia injetada')
    injetada = extrairExpressaoRegular(r'Energia compensada\s*\D*(\d+)', pag1)[0]

    print ('String da energia injetada encontrada: '+str(injetada))	

    if injetada=='none': 
        return 0 #Se não encontrar, não houve energia injetada
...
def extrairNumeroInstalacao(pag1: str) -> int:
    ...
    numero_instalacao = extrairExpressaoRegular(
        r'\d{2}\.\d{3}\.\d{3}\.\d{3}-\d{2}', pag1)[0]
    #removendo pontos e traços do número de instalação, para ficar apenas os dígitos
    numero_instalacao = numero_instalacao.replace('.', '').replace('-', '')
...
def extrairCustoDisponibilidade(pag1: str) -> float:
    ...
    '''
    #breakpoint()
    print('iniciando os trabalhos para extrair o custo de disponibilidade')
    ...
    #Testando se retornou algum valor:
    if (linha_consumo_concessionaria!="none"):
        ...
        linha_consumo_concessionaria = extrairExpressaoRegular(
            r'Custo de Disponibilidade\s+\d+,\d+', pag1)[0]
        print ('Linha consumo concessionaria corrigida: '+str(linha_consumo_concessionaria))
        valor_energia_concessionaria = extrairExpressaoRegular(
            '[0-9]+,[0-9]+', linha_consumo_concessionaria)[0]
