...
def extrairCustoDisponibilidade(pag1: str) -> float:
    ...
    Saída:
    ----------
    float contendo o valor cobrado a título de custo de disponibilidade
    ----------
    ...
    custo_disponibilidade = extrairExpressaoRegular('[0-9]+,[0-9]+',
                                                    custo_disponibilidade)[0]

    #Quando SÃO gerados créditos relacionados ao custo de disponibilidade
    if (custo_disponibilidade == 'none'):
        custo_disponibilidade = extrairExpressaoRegular(
            r'Energia Elétrica kWh\s+\d+\s+\d+,\d+\s+\d+,\d+', pag1)[0]
        custo_disponibilidade = extrairExpressaoRegular(
            '[0-9]+,[0-9]+', custo_disponibilidade)[1]
    return (custo_disponibilidade)
